...
mas = c(4); ds = c(5); d = 5; dss = 1:5; wss = 0:13
...
# User demographic group based on inferences made from users' profile
...
# When user demographics are not available
# sample randomly from distribution of users with demographic inferences
...
# Unemployment tweets: rule-based model
...
# Unemployment tweets: JoblessBERT model
...
for (c in cutoffs) {
	tw[[paste0('bert',c)]] = (tw[['bert']] > as.numeric(c)/1000) & !is.na(tw[['bert']])
}
tw[['regex']] = (tw[['regex']]==1) & !is.na(tw[['regex']])
tw = tw[,c('user_id','date','regex',paste0('bert',cutoffs)),with=FALSE] %>% group_by(user_id,date) %>% summarise_all(sum,na.rm=TRUE)
...
for (var in c('user_favourites_count','user_followers_count','user_friends_count','user_statuses_count')) {
...
tw = tw[,c('state','gender','age','date','regex',paste0('bert',cutoffs))] %>% group_by(state,gender,age,date) %>% summarise_all(sum,na.rm=TRUE)
tw = merge(tw,n_users,by=c('state','gender','age'),all.x=TRUE,all.y=TRUE)

# Population data for post-stratification
# Size of the labor force in demographic group
...
census = census[(census$SUMLEV==40) & (census$SEX!=0) & (census$AGE>=15) & (census$AGE<=50),c('NAME','SEX','age','ESTBASE2020_CIV')]
colnames(census) = c('srd_text','gender','age','pop')
census[['gender']] = (census[['gender']]==2)*1 # Female dummy
census = merge(
  census,
  read.csv(file = paste0("../preprocessing/data/srd_code_to_state.csv"),header = TRUE, sep = ",", stringsAsFactors = FALSE),
...
# Construct daily unemployment index as proportion of active users disclosing unemployment
# Reweight active users by age, gender, state based on Census Bureau data from previous month
pop_total = sum(tw[['pop']],na.rm=TRUE)
tw[['adj0_is_unemployed_regex_mrp']] = (tw[['pop']] * (tw[['regex']] / tw[['n_users']]) / pop_total)
for (c in cutoffs) {
	tw[[paste0('adj0_is_unemployed_bert',c,'_mrp')]] = (tw[['pop']] * (tw[[paste0('bert',c)]] / tw[['n_users']]) / pop_total)
}
tw_gender = tw[,c('gender','state','date','adj0_is_unemployed_regex_mrp',paste0('adj0_is_unemployed_bert',cutoffs,'_mrp'))] %>% group_by(gender,state,date) %>% summarise_all(sum,na.rm=TRUE)
tw_gender = reshape(as.data.frame(tw_gender),idvar=c('state','date'),timevar='gender',direction='wide',sep='_')
tw_age = tw[,c('age','state','date','adj0_is_unemployed_regex_mrp',paste0('adj0_is_unemployed_bert',cutoffs,'_mrp'))] %>% group_by(age,state,date) %>% summarise_all(sum,na.rm=TRUE)
tw_age[['age']] = tw_age[['age']] + 2
tw_age = reshape(as.data.frame(tw_age),idvar=c('state','date'),timevar='age',direction='wide',sep='_')
...
for (c in cutoffs) {
	tw[[paste0('adj0_is_unemployed_bert',c,'_mrp')]] = (
	  tw_agg[[paste0('adj0_is_unemployed_bert',c,'_mrp_1')]] 
	  + tw_agg[[paste0('adj0_is_unemployed_bert',c,'_mrp_2')]]
	  + tw_agg[[paste0('adj0_is_unemployed_bert',c,'_mrp_3')]] 
	  + tw_agg[[paste0('adj0_is_unemployed_bert',c,'_mrp_4')]]
	  + tw_agg[[paste0('adj0_is_unemployed_bert',c,'_mrp_5')]]
	)
}
tw_agg = data.table(tw_agg[order(tw_agg$state,tw_agg$date),])
setkey(tw_agg,state,date)
for (var in c('adj0_is_unemployed_regex_mrp',paste0('adj0_is_unemployed_bert',cutoffs,'_mrp'))) {
  for (dd in wss) {
...
cutoffs = c('500','600','700','800','900','950','990','994')
models = c("regex","bert990")
ds = c(5); spells = c(0); labels = paste0('adj',spells,'_','is_unemployed')
mas = c(4); stats = c('n_users'); statmas = paste0('n_users','_ma',mas)
...
temp0 = fread(paste0("../preprocessing/data/locations_","US","_num_state.csv"))[,c('state','num_state')] %>% unique()
p5 = tw_agg
...
dt = dt[,c('year','week','num_state',outer(paste0(stats,'_','activity',activities,'_age'),ages,paste0)),with=FALSE]
p5$week = as.numeric(p5$week) 
dt = merge(dt,p5,by=c('year','week','num_state'),all=TRUE)
...
# Fill in any gaps in time series with its own lag
...
data_iso2 = merge(data_iso2, temp1[,c("tm",uivars)], by = "tm", all.x = TRUE, all.y = TRUE)
...
data_iso2 = merge(data_iso2, temp1[,c("tm",uivars)], by = "tm", all.x = TRUE, all.y = FALSE)

# Bloomberg consensus forecasts
temp1 = read.csv(file = paste0("../preprocessing/data/restricted/ui_consensus_forecast.csv"), header = TRUE, sep = ",", stringsAsFactors = FALSE)
temp1$tm = temp1$year * 100 + temp1$week
data_iso2 = merge(data_iso2, temp1[,c("tm","consensus")], by = "tm", all.x = TRUE, all.y = FALSE)
data_iso2 = data_iso2 %>% fill(consensus)
temp1 = read_excel(paste0("../preprocessing/data/release_dates_50.xls"),range='A36:A855')
...
temp1$tm = temp1$year * 100 + temp1$week
temp1[['emp_sit']] = 1
data_iso2 = merge(data_iso2, temp1[,c("tm","emp_sit")],by = "tm", all.x = TRUE, all.y = FALSE)
...
# Fill in any missing values with lags
for (i in activities) {
  for (a in ages) {
    for (stat in stats) {
      subvar = paste0(stat,'_','activity',i,'_','age',a)
...
for (stat in stats) {
  ...
  for (a in ages) {
    ...
    for (i in activities) {
      data_iso2[[paste0(stat,'_','age',a)]] = (data_iso2[[paste0(stat,'_','age',a)]] + data_iso2[[paste0(stat,'_','activity',i,'_','age',a)]])
    }
  }
  for (a in ages) {
    ...
    data_iso2[[paste0(stat,'_ma',0)]] = (data_iso2[[paste0(stat,'_ma',0)]] + data_iso2[[paste0(subvar,'_ma',0)]])
    data_iso2[[paste0(stat,'_ma',4)]] = (data_iso2[[paste0(stat,'_ma',4)]] + data_iso2[[paste0(subvar,'_ma',4)]])
    data_iso2[[paste0(stat,'_ma',8)]] = (data_iso2[[paste0(stat,'_ma',8)]] + data_iso2[[paste0(subvar,'_ma',8)]])
  }
}

# Count unemployed users
idx = (data_iso2$tm==201840)
idx2 = (data_iso2$tm==201839)
for (c in cutoffs) {
	for (p in dss) {
	  vname = paste0('adj0_is_unemployed_bert',c,'_mrp_',p)
	  data_iso2[idx,vname] = data_iso2[idx2,vname]
	}
	for (dd in wss) {
	  vname = paste0('adj0_is_unemployed_bert',c,'_mrp_now',dd)
	  data_iso2[idx,vname] = data_iso2[idx2,vname]
	}
}
for (label in labels) {
  for (stat in statmas) {
    for (model in models) {
      for (p in dss) {
        data_iso2[[paste0(label,'_',model,'_','mrp','_',p,'_',stat)]] = (data_iso2[[paste0(label,'_',model,'_','mrp','_',p)]])
      }
      for (dd in wss) {
        data_iso2[[paste0(label,'_',model,'_','mrp','_now',dd,'_',stat)]] = (data_iso2[[paste0(label,'_',model,'_','mrp','_now',dd)]])
      }
    }
  }
}
...
write.csv(traintest, file = paste0("../preprocessing/data/restricted/data_national.csv"), row.names = FALSE)
...
d = 5; ds0 = c(1,2,3,4,5,6); pc = 1
temp1 = fread(paste0("../preprocessing/data/ui_state_iso2_","US",".csv"))
temp1 = temp1[temp1$year>=2016,c('state','iclaimnsa')] %>% group_by(state) %>% summarise_all(mean,na.rm=TRUE)
pf = fread('../preprocessing/data/restricted/latest_profiles.csv')
pf[['tweet_timestamp']] = as.Date(as.POSIXct(pf[['tweet_timestamp']],origin='1970-01-01'))
pf[['acctage']] = 2022 - year(pf[['tweet_timestamp']])
pf[['tweet_timestamp']] = NULL
...
tw[['regex']] = (tw[['regex']]==1) & !is.na(tw[['regex']])
tw = tw[,c('user_id','year','week','regex',paste0('bert',cutoffs)),with=FALSE] %>% group_by(user_id,year,week) %>% summarise_all(sum,na.rm=TRUE)
tw = merge(tw,pf,by=c('user_id'),all.x=TRUE,all.y=FALSE)
tw[['user_statuses_count']] = tw[['user_statuses_count']] / (1+tw[['acctage']])
for (var in c('gender','age','org' )) {
  tw[is.na(tw[[var]]),var] = -1
...
tw = tw[tw[['state']]!="",]
tw = tw[!is.na(tw[['gender']]),]
tw = tw[,c('state','gender','age','year','week','regex',paste0('bert',cutoffs))] %>% group_by(state,gender,age,year,week) %>% summarise_all(sum,na.rm=TRUE)
tw = merge(tw,n_users,by=c('state','gender','age'),all.x=TRUE,all.y=TRUE)
census = as.data.frame(fread('../preprocessing/data/sc-est2022-agesex-civ.csv'))
census[['age']] = NA
census[(census[['AGE']]>=15) & (census[['AGE']]<=20),'age'] = 0
...
# construct daily unemployment index 
# as the proportion of active users disclosing their unemployment status
tw[['adj0_is_unemployed_regex_mrp']] = (tw[['regex']] / tw[['n_users']])
for (c in cutoffs) {
  tw[[paste0('adj0_is_unemployed_bert',c,'_mrp')]] = (tw[[paste0('bert',c)]] / tw[['n_users']])
}
tw_gender = tw[,c('gender','state','year','week','adj0_is_unemployed_regex_mrp',paste0('adj0_is_unemployed_bert',cutoffs,'_mrp'))] %>% group_by(gender,state,year,week) %>% summarise_all(sum,na.rm=TRUE)
tw_gender = reshape(as.data.frame(tw_gender),idvar=c('state','year','week'),timevar='gender',direction='wide',sep='_')
tw_age = tw[,c('age','state','year','week','adj0_is_unemployed_regex_mrp',paste0('adj0_is_unemployed_bert',cutoffs,'_mrp'))] %>% group_by(age,state,year,week) %>% summarise_all(sum,na.rm=TRUE)
tw_age[['age']] = tw_age[['age']] + 2
...
lagmaxs = c(4); ds = c(5)
cutoffs = c('500','600','700','800','900','950','990','994')
models = c("regex",paste0("bert",cutoffs))
spells = c(0); labels = c(paste0('adj',spells,'_','is_unemployed'))
mas = c(4); stats = c('n_users'); statmas = paste0('n_users','_ma',mas)
...
# Bloomberg consensus forecast data
temp1 = read.csv(file = paste0("../preprocessing/data/restricted/ui_consensus_forecast.csv"), header = TRUE, sep = ",", stringsAsFactors = FALSE)
temp1$tm = temp1$year * 100 + temp1$week
data_iso2 = merge(data_iso2, temp1[,c("tm","consensus")], by = "tm", all.x = TRUE, all.y = FALSE)
data_iso2 = data_iso2 %>% fill(consensus)
...
data_iso2[['dummy_covid']] = (data_iso2$tm>=covidstart & data_iso2$tm<=covidend)
data_iso2 = as.data.frame(data_iso2)
traintest = data_iso2[,c('tm','dummy_covid',uivars,'consensus','emp_sit','eip',statmas, outer(outer(outer(labels,paste0('_',models,'_'),paste0),paste0('mrp_',dss,'_','n_users','_ma'),paste0),mas,paste0))]
uw = traintest

# Post-stratified unemployment index
mrp = read.csv(file = paste0("../preprocessing/data/restricted/data_national.csv"), header = TRUE, sep = ",", stringsAsFactors = FALSE)
mrp[['regex_mrp']] = (
...
dt = merge(mrp[,c('tm','regex_mrp','bert990_mrp')], uw[,c('tm','regex_uw','bert990_uw')], by = "tm", all.x = TRUE, all.y = TRUE)
dt = merge(dt, temp1[,c("tm","iclaimnsa")],by = "tm", all.x = TRUE, all.y = TRUE)
dt = dt[order(dt$tm),]
dt[['iclaimnsa_raw']] = dt[['iclaimnsa']]
dt0 = dt

write.csv(dt0,file='time_series.csv',row.names=FALSE)
...
cutoffs = c('500','600','700','800','900','950','990','994')
models = c("regex","bert990")
spells = c(0); labels = c('adj0_is_unemployed')
...
colnames(dt) = sub("age-1","age4",colnames(dt))
dt = dt[,c('year','week','num_state',outer(paste0(stats,'_age'),ages,paste0)),with=FALSE]
p5$week = as.numeric(p5$week) 
dt = merge(dt,p5,by=c('year','week','num_state'),all=TRUE)
dt$tm = dt$year * 100 + dt$week
dt[dt$tm==201653,'tm'] = 201652
...
# National level data
data_iso2 = read.csv(file = paste0("../preprocessing/data/restricted/data_national.csv"),header = TRUE, sep = ",", stringsAsFactors = FALSE)

# Create lags of predictors and interact with covid dummy
for (var in c(uivars,'consensus')) {
  for (lag in 0:(max(lagmaxs)+2)) {
    ...
    data_iso2[[paste0('iso2_L',lag,'_',var,'_','dummy_covid')]] = (data_iso2[[paste0('iso2_L',lag,'_',var)]] * data_iso2[['dummy_covid']])
  }
}
for (var in c(
  outer(outer(outer(labels,paste0('_',models,'_'),paste0),paste0('mrp_',dss,'_','n_users','_ma'),paste0),mas,paste0),
...
data_iso2 = data_iso2[,c('tm','emp_sit','eip',colnames(data_iso2)[grepl('iso2_L',colnames(data_iso2))])]
...
dt[[uivars]] = NULL
dt = merge(dt, temp1[,c('tm','state',uivars),with=FALSE], by = c('tm','state'), all.x = TRUE, all.y = FALSE)
...
for (stat in stats) {
  ...
  for (i in ages) { # 
    ...
    dt[[paste0(stat,'_ma',0)]] = (dt[[paste0(stat,'_ma',0)]] + dt[[paste0(subvar,'_ma',0)]])
    dt[[paste0(stat,'_ma',4)]] = (dt[[paste0(stat,'_ma',4)]] + dt[[paste0(subvar,'_ma',4)]])
    dt[[paste0(stat,'_ma',8)]] = (dt[[paste0(stat,'_ma',8)]] + dt[[paste0(subvar,'_ma',8)]])
  }
}
setkey(dt,state,tm)
for (label in labels) {
  for (stat in statmas) {
    for (model in models) {
      for (p in dss) {
        dt[[paste0(label,'_',model,'_','mrp','_',p,'_',stat)]] = (dt[[paste0(label,'_',model,'_','mrp','_',p)]])
      }
      for (dd in wss) {
        dt[[paste0(label,'_',model,'_','mrp','_now',dd,'_',stat)]] = (dt[[paste0(label,'_',model,'_','mrp','_now',dd)]])
      }
    }
  }
}
...
# Interact predictors with covid dummy
for (var in c(
  uivars,
  outer(outer(outer(labels,paste0('_',models,'_'),paste0),paste0('mrp_',dss,'_','n_users','_ma'),paste0),mas,paste0),
  outer(outer(outer(labels,paste0('_',models,'_'),paste0),paste0('mrp_now',wss,'_','n_users','_ma'),paste0),mas,paste0)
  ...
  for (lag in 0:(max(lagmaxs)+2)) {
    ...
    dt[[paste0('L',lag,'_',var,'_','dummy_covid')]] = (dt[[paste0('L',lag,'_',var)]] * dt[['dummy_covid']])
  }
}
dt = as.data.frame(dt)
dt$year = floor(dt$tm / 100)
...
dt = merge(dt,data_iso2,by='tm',all.x=TRUE,all.y=FALSE)
dt = dt[order(dt$state,dt$tm),]
...
cities_long = dt[!is.na(dt[[uivars]]) & dt$tm %in% dates_to_cover,c('state','tm','n_users_ma4')]
cities_long$count = 1
cities_long = cities_long[,c('state','count','n_users_ma4')] %>% group_by(state) %>% summarise_all(sum,na.rm=TRUE)
...
dt = merge(dt,cities_long,by='state',all.x=TRUE,all.y=FALSE)

data = dt[dt$tm>=201601,]
data = data[!is.na(data$state) & !is.na(data[[uivars]]) & !is.na(data$tm),]
data = data[order(data$state,data$tm),]

write.csv(data, file = paste0("../preprocessing/data/restricted/data_state.csv"),row.names = FALSE)

#######################################################
## City level

# Maps profile locations to cities
locations = fread(paste0("../preprocessing/data/locations_","US",".csv"))[,c('user_location','geo_id','metro_area_name')] %>% unique()
temp1 = fread(paste0("../preprocessing/data/ui_state_iso2_","US",".csv"))
temp1 = temp1[temp1$year>=2016,c('state','iclaimnsa')] %>% group_by(state) %>% summarise_all(mean,na.rm=TRUE)
pf_locations = fread('../preprocessing/data/restricted/latest_profiles_user_location.csv')

# User profile information
pf = merge(fread('../preprocessing/data/restricted/latest_profiles.csv'),pf_locations,by=c('user_id'),all.x=TRUE,all.y=FALSE)
...
# Number of users
n_users = pf[,c('state','geo_id','age','gender','n_users')] %>% group_by(state,geo_id,age,gender) %>% summarise_all(sum,na.rm=TRUE) 
n_users = n_users[n_users[['state']]!="",]
n_users = n_users[!is.na(n_users[['gender']]),]
n_gender = n_users[,c('gender','n_users')] %>% group_by(gender) %>% summarise_all(sum,na.rm=TRUE)
n_gender[['n_users']] = n_gender[['n_users']] / sum(n_gender[['n_users']],na.rm=TRUE)
...
tw = tw[tw[['state']]!="",]
tw = tw[!is.na(tw[['gender']]),]
tw = tw[,c('state','geo_id','gender','age','date','regex',paste0('bert',cutoffs))] %>% group_by(state,geo_id,gender,age,date) %>% summarise_all(sum,na.rm=TRUE)
...
tw_gender = tw[,c('gender','geo_id','date','adj0_is_unemployed_regex_mrp',paste0(paste0('adj0_is_unemployed_bert',cutoffs,'_mrp')))] %>% group_by(gender,geo_id,date) %>% summarise_all(sum,na.rm=TRUE)
tw_gender = reshape(as.data.frame(tw_gender),idvar=c('geo_id','date'),timevar='gender',direction='wide',sep='_')
tw_age = tw[,c('age','geo_id','date','adj0_is_unemployed_regex_mrp',paste0(paste0('adj0_is_unemployed_bert',cutoffs,'_mrp')))] %>% group_by(age,geo_id,date) %>% summarise_all(sum,na.rm=TRUE)
tw_age[['age']] = tw_age[['age']] + 2
tw_age = reshape(as.data.frame(tw_age),idvar=c('geo_id','date'),timevar='age',direction='wide',sep='_')
...
for (var in c(paste0('adj0_is_unemployed_regex_mrp'),paste0(paste0(paste0('adj0_is_unemployed_bert',cutoffs,'_mrp'))))) {
  for (dd in wss) {
...
# Actual city level UI claims data
ui = fread("../preprocessing/data/ui_city.csv")
ui = merge(ui,fread(paste0("../preprocessing/data/locations_","US",".csv"))[,c('geo_id','metro_area_name')] %>% unique(),by='metro_area_name',all.x=TRUE,all.y=FALSE)
ui = ui[!is.na(ui$geo_id),]
p5 = tw_agg
dt = fread(paste0("../preprocessing/data/n_users_city_week_demo_US.csv"))
colnames(dt) = sub("age-1","age4",colnames(dt))
dt = merge(dt,ui,by=c("geo_id","year","week"),all.x=FALSE,all.y=TRUE)
dt = dt[,c('year','week','metro_area_name','geo_id',outer(paste0(stats,'_age'),ages,paste0)),with=FALSE]
p5$week = as.numeric(p5$week) 
dt = merge(dt,p5,by=c('year','week','geo_id'),all=TRUE)
dt$tm = dt$year * 100 + dt$week
dt[dt$tm==201653,'tm'] = 201652
dt[dt$tm==201753,'tm'] = 201752
dt[dt$tm==201853,'tm'] = 201852
...
dt[['year']] = floor(dt[['tm']] / 100)
dt[['week']] = dt[['tm']] - dt[['year']] * 100
dt = dt %>% group_by(metro_area_name,tm,year,week) %>% summarise_all(sum,na.rm=TRUE)
...
# Create lags of predictors and interact with covid dummy
for (var in c('iclaimnsa','consensus')) {
...
# City level actual UI claims data
temp1 = fread(paste0("../preprocessing/data/ui_city.csv"))
temp1$tm = temp1$year * 100 + temp1$week
dt[['iclaimnsa']] = NULL
dt = merge(dt, temp1[,c('tm','metro_area_name','iclaimnsa'),with=FALSE],by = c('tm','metro_area_name'), all.x = TRUE, all.y = FALSE)
...
dt = merge(dt,locations,by=c('metro_area_name'),all.x=TRUE,all.y=FALSE)

dt = dt[order(dt$metro_area_name,dt$tm),]
dt = dt[
  ,c(
    'tm','metro_area_name','dummy_covid','iclaimnsa','emp_sit','eip',statmas,
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),'iclaimnsa',paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_'),dss,paste0),paste0),paste0('_n_users_ma',mas),paste0),paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_now'),wss,paste0),paste0),paste0('_n_users_ma',mas),paste0),paste0),
    outer(paste0('iso2_L',0:(max(lagmaxs)+1),'_'),'iclaimnsa',paste0),
    outer(paste0('iso2_L',0:(max(lagmaxs)+1),'_'),'consensus',paste0),
    outer(paste0('iso2_L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_'),dss,paste0),paste0),paste0('_n_users_ma',mas),paste0),paste0),
    outer(paste0('iso2_L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_now'),wss,paste0),paste0),paste0('_n_users_ma',mas),paste0),paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),paste0('iclaimnsa','_','dummy_covid'),paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_'),dss,paste0),paste0),paste0('_n_users_ma',mas,'_','dummy_covid'),paste0),paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_now'),wss,paste0),paste0),paste0('_n_users_ma',mas,'_','dummy_covid'),paste0),paste0),
    ...
    outer(paste0('iso2_L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_'),dss,paste0),paste0),paste0('_n_users_ma',mas,'_','dummy_covid'),paste0),paste0),
    outer(paste0('iso2_L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_now'),wss,paste0),paste0),paste0('_n_users_ma',mas,'_','dummy_covid'),paste0),paste0)
  )]
...
data_state = data_state[
  ,c(
    'tm','state','dummy_covid','iclaimnsa','emp_sit','eip',statmas,
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),'iclaimnsa',paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_'),dss,paste0),paste0),paste0('_n_users_ma',mas),paste0),paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_now'),wss,paste0),paste0),paste0('_n_users_ma',mas),paste0),paste0),
    outer(paste0('iso2_L',0:(max(lagmaxs)+1),'_'),'iclaimnsa',paste0),
...
# Divide cities into trained and holdout cities
# Based on the availability of ground truth data on UI claims
dates_to_cover = c(201902,202002,202102,202202)
cities_long = dt[!is.na(dt[['iclaimnsa']]) & dt$tm %in% dates_to_cover,c('metro_area_name','tm','n_users_ma4')]
cities_long$count = 1
cities_long = cities_long[,c('metro_area_name','count','n_users_ma4')] %>% group_by(metro_area_name) %>% summarise_all(sum,na.rm=TRUE)
cities_long[['sample']] = (cities_long$count==length(dates_to_cover))

# Trained cities are cities with at least one year of data available before 2020 week 1
# Otherwise we set aside separate set of holdout cities not used to estimate predictive model
cities_long = cities_long[cities_long$count > 1,]
cities_long[['ntile_n_users_ma4']] = ntile(cities_long[['n_users_ma4']],10)
cities_long[['n_users_ma4']] = NULL
dt[['count']] = NULL
dt[['sample']] = NULL
dt[['ntile_n_users_ma4']] = NULL
dt = merge(dt,cities_long,by='metro_area_name',all.x=TRUE,all.y=FALSE)

data = dt[
  dt$tm>=201601
  ,c(
    'tm','sample','ntile_n_users_ma4','metro_area_name','dummy_covid','iclaimnsa','emp_sit',statmas,
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),'iclaimnsa',paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_'),dss,paste0),paste0),paste0('_n_users_ma',mas),paste0),paste0),
    outer(paste0('L',0:(max(lagmaxs)+1),'_'),outer(outer(labels,outer(paste0('_',models,'_','mrp_now'),wss,paste0),paste0),paste0('_n_users_ma',mas),paste0),paste0),
    outer(paste0('iso2_L',0:(max(lagmaxs)+1),'_'),'iclaimnsa',paste0),
...
write.csv(data,file = paste0("../preprocessing/data/restricted/data_city.csv"),row.names = FALSE)
