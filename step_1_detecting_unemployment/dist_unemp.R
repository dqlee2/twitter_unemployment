...
tw[['regex']] = (tw[['regex']]==1) & !is.na(tw[['regex']])
tw = tw[,c('user_id','date','regex',paste0('bert',990)),with=FALSE] %>% group_by(user_id,date) %>% summarise_all(sum,na.rm=TRUE)
...
tw[['date']] = NULL
tw[['tm']] = NULL
tw = tw[,c('state','gender','age','year','week','regex',paste0('bert',990))] %>% group_by(state,gender,age,year,week) %>% summarise_all(sum,na.rm=TRUE)
...
dist = dist[,c(
  'state','tm','year','week','n_users',paste0('n_users_age',0:3),paste0('n_users_gender',0:1),
  'regex','bert990',paste0('regex_age',0:3),paste0('bert990_age',0:3),paste0('regex_gender',0:1),paste0('bert990_gender',0:1)
)]
...
dist = merge(dist, temp1[,c('tm','state','iclaimnsa'),with=FALSE],by = c('tm','state'), all.x = TRUE, all.y = FALSE)
...
colnames(demo) = c(
  'tm','u','ufull','upart','u1617','u1619','ufull1619','upart1619','u1624','u1819','u20p','u2024','u25p','u2534','u2554','u3544','u4554','u55p',
  'umales','umaleu','umalefulls','umalefullu','umaleparts','umalepartu','ufemales','ufemaleu','ufemalefulls','ufemalefullu','ufemaleparts','ufemalepartu'
)
...
demo = demo[,c('year','ui_female','ui_male','ui_age_lt20','ui_age_2030','ui_age_3040','ui_age_gt40')]
demo = demo %>% group_by(year) %>% summarise_all(sum,na.rm=TRUE)
dist = merge(dist, demo,by = c('year'), all.x = TRUE, all.y = FALSE)

# Population size
census = read_excel(paste0("../preprocessing/data/NST-EST2021-POP.xlsx"),range = "A4:D60")
census = census[,c(1,3)]
...
dist = merge(dist, census,by = 'state', all.x = TRUE, all.y = FALSE)

#######################################################
# Distribution of unemployed users by state

idx = (dist$year>=2020 & dist$year<=2021 & !is.na(dist[['iclaimnsa']]) & dist[['iclaimnsa']] > 0 & !is.na(dist[[paste0('bert990')]]) & dist[[paste0('bert990')]] > 0)
demo = dist[idx,c('state','iclaimnsa','regex','bert990')] 
nrows = nrow(dist)
...
idx = (dist$year>=2020 & dist$year<=2021 & !is.na(dist[['ui_age_lt20']]) & dist[['ui_age_lt20']] > 0 & !is.na(dist[[paste0('bert990_age',0)]]) & dist[[paste0('bert990_age',0)]] > 0)
demo = dist[idx,c('state','pop','ui_age_lt20','ui_age_2030','ui_age_3040','ui_age_gt40',paste0('regex_age',0:3),paste0('bert990_age',0:3))]
nrows = nrow(dist)
demo[['state']] = 'US'
demo_sd = demo %>% group_by(state) %>% summarise_all(sd,na.rm=TRUE)
demo = demo %>% group_by(state) %>% summarise_all(sum,na.rm=TRUE)
...
colnames(demo_sd) = colnames(demo)
ui = demo[,c('state','ui_age_lt20','ui_age_2030','ui_age_3040','ui_age_gt40')]
colnames(ui) = c('state','lt20','2030','3040','gt40')
ui = data.table::melt(data.table(ui),id.var='state')
colnames(ui) = c('state','age','ui')
ui_sd = demo_sd[,c('state','ui_age_lt20','ui_age_2030','ui_age_3040','ui_age_gt40')]
...
regex = demo[,c('state','regex_age_lt20','regex_age_2030','regex_age_3040','regex_age_gt40')]
...
regex_sd = demo_sd[,c('state','regex_age_lt20','regex_age_2030','regex_age_3040','regex_age_gt40')]
...
bert = demo[,c('state','bert_age_lt20','bert_age_2030','bert_age_3040','bert_age_gt40')]
...
bert_sd = demo_sd[,c('state','bert_age_lt20','bert_age_2030','bert_age_3040','bert_age_gt40')]
...
idx = (dist$year>=2020 & dist$year<=2021 & !is.na(dist[['ui_female']]) & dist[['ui_female']] > 0 & !is.na(dist[[paste0('bert990_age',0)]]) & dist[[paste0('bert990_age',0)]] > 0)
demo = dist[idx,c('state','pop','ui_female','ui_male',paste0('regex_gender',0:1),paste0('bert990_gender',0:1))] 
nrows = nrow(dist)
demo[['state']] = 'US'
demo_sd = demo %>% group_by(state) %>% summarise_all(sd,na.rm=TRUE)
demo = demo %>% group_by(state) %>% summarise_all(sum,na.rm=TRUE)
colnames(demo) = c('state','pop','ui_female','ui_male','regex_female','regex_male','bert_female','bert_male')
colnames(demo_sd) = colnames(demo)
